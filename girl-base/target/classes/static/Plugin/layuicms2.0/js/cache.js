...
layui.use(['form','jquery',"layer"],function() {
    ...
    function lockPage(){
        layer.open({
            title : false,
            type : 1,
            content : '<div class="sys-header-lock" id="lock-box">'+
                            '<div class="sys-header-lock-img"><img src="images/face.jpg" class="userAvatar"/></div>'+
                            '<div class="sys-header-lock-name" id="lockUserName">驊驊龔頾</div>'+
                            '<div class="input_btn">'+
                                '<input type="password" class="sys-header-lock-input layui-input" autocomplete="off" placeholder="请输入密码解锁.." name="lockPwd" id="lockPwd" />'+
    ...
    $("body").on("click","#unlock",function(){
        if($(this).siblings(".sys-header-lock-input").val() == ''){
            layer.msg("请输入解锁密码！");
            $(this).siblings(".sys-header-lock-input").focus();
        }else{
            if($(this).siblings(".sys-header-lock-input").val() == "123456"){
                window.sessionStorage.setItem("lockcms",false);
                $(this).siblings(".sys-header-lock-input").val('');
                layer.closeAll("page");
            }else{
                layer.msg("密码错误，请重新输入！");
                $(this).siblings(".sys-header-lock-input").val('').focus();
            }
        }
    });
    ...
    function skins(){
        ...
        if(skin){  //如果更换过皮肤
            if(window.sessionStorage.getItem("skinValue") != "自定义"){
                ...
                $(".layui-layout-sys .layui-header").css("background-color",skin.split(',')[0]);
            ...
            success : function(index, layero){
                ...
                $(".skins_box .layui-form-radio").on("click",function(){
                    ...
                    if($(this).find("div").text() != "自定义"){
                        ...
                        $(".layui-bg-black,.hideMenu,.layui-layout-sys .layui-header").removeAttr("style");
                    }else{
                        $(".skinCustom").css("visibility","inherit");
                    }
                })
                ...
                $(".topColor").blur(function(){
                    $(".layui-layout-sys .layui-header").css("background-color",$(this).val()+" !important");
                })
