...
public class RedisUtils {
    ...
    /**
     *@Description  清空redis 字典缓存数据
     *@Date 2018/3/26 15:41
     *@Author jinghan
     */
    public String flushDB() {
        //获取所有 字典 的key
        Set<String> keys = redisTemplate.keys(Const.REDIS_P_BM + "*");
    ...
    /**
     * 缓存ZSet追加数据 添加指定系统时间排序
     *
     * @param key      缓存的键值
     * @param obj 待缓存的pd数据
     * @param score 排序字段 可以为空
     * @return 缓存的对象
     */
    public void addCacheZSet(String key, Object obj, Long score) {
        // 获取已缓存的最近浏览的职位
        ZSetOperations<String, Object> zSetOperations = redisTemplate.opsForZSet();
        if(Utils.isNotNull(obj)){
            ...
            //判断条件 添加怎样类型的value值 后期继续添加其他类型
            if(obj instanceof  PageData){
                PageData pd = (PageData)obj;
                pd.put("score",score);
                zSetOperations.add(key, pd, score);
            }else{
